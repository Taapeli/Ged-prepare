...
def process_neo4j(args):
    referenssinimet(args.csv_file)

def main():
    parser = argparse.ArgumentParser(description='Store reference names from csv file to Neo4j')
    parser.add_argument('csv_file', help="Name of the CSV file")

    if len(sys.argv) == 1:
        print("First argument must be the name of the CSV file")
        return
